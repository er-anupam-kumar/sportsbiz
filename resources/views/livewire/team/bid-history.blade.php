        ...
        @foreach($bids as $bid)
            <div class="p-3 flex items-center justify-between gap-3">
                <div class="flex items-center gap-2">
                    <img src="{{ $bid->player?->image_path ? asset('storage/'.$bid->player->image_path) : asset('images/team-placeholder.svg') }}" alt="{{ $bid->player?->name ?? 'Player' }}" class="h-8 w-8 rounded-md object-cover border border-slate-200">
