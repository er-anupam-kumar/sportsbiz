                    ...
                    <tr class="border-b last:border-b-0">
                        ...
                        <td class="p-3" x-data="{open:false}">
                            ...
                            <div x-show="open" @click.outside="open=false" class="absolute mt-1 bg-white border rounded shadow z-10" x-cloak>
                                <button wire:click="edit({{ $sport->id }})" class="block w-full text-left px-3 py-2 text-sm hover:bg-slate-50">Edit</button>
                                <button wire:click="toggle({{ $sport->id }})" class="block w-full text-left px-3 py-2 text-sm hover:bg-slate-50">Toggle Status</button>
                                <button wire:click="delete({{ $sport->id }})" wire:confirm="Delete this sport?" class="block w-full text-left px-3 py-2 text-sm text-red-600 hover:bg-red-50">Delete</button>
                            </div>
                        </td>
                    </tr>
                @empty
