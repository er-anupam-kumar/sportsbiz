...
<body class="min-h-screen sb-shell-bg text-slate-900 overflow-x-hidden" x-data="{ mobileSidebar: false, desktopSidebarOpen: true, toast: '' }" x-on:toast.window="toast = $event.detail.message; setTimeout(() => toast = '', 2200)">
    @php
        ...
        $activeSection = $isActive(['team.auction-room', 'team.squad', 'team.bid-history', 'team.fixtures', 'team.jersey-requirements']) ? 'auction' : 'overview';
    @endphp
                ...
                @if($teamTournamentId)
                    <div class="space-y-1">
                        ...
                        <div x-show="activeSection === 'auction'" class="space-y-1">
                            ...
                            <a href="{{ route('team.fixtures') }}" class="sb-nav-link {{ $isActive(['team.fixtures']) ? 'bg-white/20' : '' }}"><i data-lucide="calendar-days" class="w-4 h-4"></i>Fixtures</a>
